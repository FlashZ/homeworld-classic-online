...
def test_build_auth1_login_failure_reply_round_trip() -> None:
    frame = won_crypto.build_auth1_login_failure_reply(won_crypto.STATUS_AUTH_USER_NOT_FOUND)
    service_type, message_type, body = won_crypto.parse_tmessage(frame[4:])

    assert service_type == won_crypto.AUTH1_SERVICE_TYPE
    assert message_type == won_crypto.AUTH1_LOGIN_REPLY
    assert body == struct.pack("<hBB", won_crypto.STATUS_AUTH_USER_NOT_FOUND, 0, 0)


def test_parse_auth1_login_request_uses_remaining_bytes_for_bf_data() -> None:
    eg_ciphertext = b"\xAA" * 12
    bf_ciphertext = b"\x10\x20\x30\x40\x50\x60\x70\x80"
    body = struct.pack("<HH", 9, len(eg_ciphertext)) + eg_ciphertext + bf_ciphertext

    parsed = won_crypto.parse_auth1_login_request(body)

    assert parsed["block_id"] == 9
    assert parsed["eg_ciphertext"] == eg_ciphertext
    assert parsed["bf_data"] == bf_ciphertext
...
def test_parse_auth1_login_payload_round_trip() -> None:
    ...
    assert parsed["ciphertext_variant"] == "direct"


def test_parse_auth1_login_payload_decodes_raw_binary_cd_key() -> None:
    session_key = b"HWLOGIN!"
    raw_cd_key = won_crypto.cd_key_from_display("NYX7-ZEC9-FYZ6-GUX8-4253")
    raw_login_key = b"\x01\x23\x45\x67\x89\xAB\xCD\xEF"

    def _pw(text: str) -> bytes:
        return struct.pack("<H", len(text)) + text.encode("utf-16-le")

    clear = (
        struct.pack("<HBB", 1, 1, 0)
        + _pw("&Z&e&r&o|&S&F")
        + _pw("Homeworld")
        + _pw("")
        + _pw("secret")
        + _pw("")
        + struct.pack("<H", len(raw_cd_key))
        + raw_cd_key
        + struct.pack("<H", len(raw_login_key))
        + raw_login_key
    )
    encrypted = won_crypto.bf_encrypt(clear, session_key)
    ...
    assert parsed["username"] == "&Z&e&r&o|&S&F"
    assert parsed["community_name"] == "Homeworld"
    assert parsed["cd_key"] == "NYX7-ZEC9-FYZ6-GUX8-4253"
    assert parsed["raw_cd_key"] == raw_cd_key
    assert parsed["login_key"] == raw_login_key.hex()


def test_parse_auth1_login_payload_accepts_wrapped_retail_variant() -> None:
    session_key = b"HWLOGIN!"
    ...
    clear = (
        ...
        + _pw("secret")
        + _pw("")
        + _raw("NYX7ZEC9FYZ6GUX84253")
        + _raw("")
    )
    encrypted = won_crypto.bf_encrypt(clear, session_key)
    wrapped = struct.pack("<H", len(encrypted)) + encrypted + b"\x12\x34\x56\x78"

    parsed = won_crypto.parse_auth1_login_payload(wrapped, session_key)

    assert parsed["block_id"] == 1
    assert parsed["community_name"] == "Sierra"
    assert parsed["password"] == "secret"
    assert parsed["cd_key"] == "NYX7ZEC9FYZ6GUX84253"
    assert parsed["ciphertext_variant"] == "len2_suffix4"


def test_parse_auth1_login_payload_accepts_big_endian_wrapped_retail_variant() -> None:
    session_key = b"HWLOGIN!"
    ...
    encrypted = won_crypto.bf_encrypt(clear, session_key)
    wrapped = struct.pack(">H", len(encrypted)) + encrypted + b"\xDE\xAD\xBE\xEF"

    parsed = won_crypto.parse_auth1_login_payload(wrapped, session_key)

    assert parsed["username"] == "Zero"
    assert parsed["community_name"] == "Sierra"
    assert parsed["password"] == "secret"
    assert parsed["cd_key"] == "NYX7ZEC9FYZ6GUX84253"
    assert parsed["ciphertext_variant"] == "len2be_suffix4"
